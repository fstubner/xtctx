...
One more is now reproducible: the three-OS device table, which
...
comparison, the segment-duplication count, and the bge/gte rows — was measured
in one session on one machine with scratch scripts that are not in this
repository. The method
...
**`auto` is `cpu`, on all three.** That is what the product passes today, so
the GPU is not merely underused — it is unused, and the desktop measurement
above is not being partially realised by anyone.
