...
export async function runServe(options: ServeOptions = {}): Promise<void> {
  ...
  if (!options.mcpOnly) {
    console.error(
      "xtctx serve: ingestion daemon and web UI are not implemented yet; starting MCP server only.",
    );
  }

  console.error(`xtctx serve: MCP stdio server active for project ${projectRoot}`);
